...
public class LanTesterCrossoverPower : MonoBehaviour
{
    [Header("Scripts References")]
    // 🔥 SUDAH DIGANTI: Sekarang memanggil target manager Crossover
    public LanTesterCrossoverManager testMgr; 
    ...
    {
        isKabelPlugged = true;
        Debug.Log("CONSOLE: Kabel terdeteksi! Siap dinyalakan (Crossover).");
    }
    ...
    public void TogglePowerState()
    {
        ...
        {
            // PROSES ON
            Debug.Log("CONSOLE: Power ON! (Crossover)");
            ...
            // Memanggil sequence Crossover
            if(testMgr != null) testMgr.MulaiSequence(true); 
        }
        else
        {
            // PROSES OFF
            Debug.Log("CONSOLE: Power OFF! (Crossover)");
    ...
    public void ResetLanTesterPower()
    {
        ...
        SetLEDAlpha(0f);
        Debug.Log("CONSOLE: LanTesterCrossoverPower berhasil di-reset ke posisi OFF!");
    }
}